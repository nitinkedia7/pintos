...
/* Store the priority of current thread and increase its priority. */
void
...
bool
before_thread(const struct list_elem *a,
...
/* Compares the wakeup time of two thread A and B.
   Returns true if A is less than B, or
   false if A is greater than or equal to B. */
bool
before(const struct list_elem *a,
      const struct list_elem *b,
      void *aux UNUSED){
        struct thread *s1 = list_entry(a, struct thread,elem);
        struct thread *s2 = list_entry(b, struct thread,elem);
...
/* Sleeps for approximately TICKS timer ticks.
   Disables interrupts and increases to max priority
   while the current thread is going to sleep. */
void
